...
class Slave(username: String) extends ImplicitLazyLogger {
  ...
  def joinRequestCloriko(password: String): Task[JoinReply] = {
    logger.info(s"Slave - Received join request at slave: $slaveId of user: $username")
        ...
        case Some(file) => {
          FileSystem.createFile(file).runAsync.onSuccess {
            case true => {
              ...
              (s"Slave - Update received: $update, returning Updated event, slaveResponseUpstream used: $slaveUpStream")
            }
            case false => logger.info(s"There was a failure during the creation of file from update: ${update.id} at slave $slaveId")
          }
        }
        case None => {
          logger.info(s"Slave - Update file ${update.id} was empty... dir to do for ${slaveId} ")
          slaveUpStream.onNext(Updated(update.id, username, slaveId).asProto)
        }
      }
    }
    ...
    Task.eval {
      FileSystem.deleteFile(delete.references.head).runAsync.onSuccess {
        case true => {
          ...
          logger.info(s"Slave - Update received: $delete, returning Updated event, slaveResponseUpstream used: $upStream")
        }
        case false => logger.info(s"There was a failure during the creation of file from update: ${delete.id} at slave $slaveId")

      }
    }
  }

  def handleFetch(implicit fetchRequest: FetchRequest, upStream: Subscriber.Sync[SlaveResponse]): Task[Unit] = {
    Task.eval {
      FileSystem.scanFile(FileReference(fetchRequest.fileName, fetchRequest.path)).runAsync.onSuccess {
        case file: File =>
          ...
          ctxLogger.info(s"Slave - Fetch response sent: $fetchResponse to upStream channel: $upStream")
      }
    }
  }
        ...
        sealedValue.update match {
          ...
          case _ => sealedValue.delete match {
            ...
            case _ => sealedValue.fetchRequest match {
              ...
              case _ => logger.info(s"Slave - A master request operation not yet implemented was received. sealedValue $sealedValue")
  ...
  def initProtocol(username: String, slaveId: String) = {
    logger.info(s"Slave - Initializing update flow from slave $slaveId to $username cluster")
    ...
    val ob = stub.protocol {
      val obs = Observable.create[SlaveResponse](OverflowStrategy.Unbounded) {
        upStream: Subscriber.Sync[SlaveResponse] =>
          logger.info(s"Slave - UpdatedUpStream created: $upStream and actioned")
          ...
          Task.now(logger.info("Slave - Running dummy task")).runAsync
      }
      //obs.runAsyncGetFirst
      logger.info(s"Slave - Update stream protocol called, observable created: $obs")
      obs
    }
    ...
    consumerStarter.runAsync.onSuccess {
      case _: MasterRequest => {
        ...
        logger.info(s"Slave - Starting to consume update events from updateUpstream ${grpcChannel.get.upStream}")
        ob.consumeWith(masterRequestsConsumer(currentUpStream)).runAsync
        //ob.consumeWith(updateCousumer(currentUpstream)).runAsync
      }
      case _ => logger.info("Slave - Update consumer not started... SOmething failed")
    }
