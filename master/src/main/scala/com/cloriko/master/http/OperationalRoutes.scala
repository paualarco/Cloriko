...
trait OperationalRoutes extends ImplicitLazyLogger with Generators {
  ...
  lazy val operationalRoutes: HttpRoutes[IO] = HttpRoutes.of[IO] {
      ...
      req.decode[Multipart[IO]] { multipart =>
        ...
        parts.find(_.name == "file".some) match {
          case Some(file) => {
            ...
            logger.info(s"Multipart update received ${update}")
    ...
    case req @ POST -> Root / "delete" => {
      implicit val delete: Delete = req.as[Delete].unsafeRunSync()
      ctxLogger.info(s"WebServer - Delete operation received from user ${delete.username}")
    ...
    case req @ POST -> Root / "fetch" => {
      implicit val fetchRequest: FetchRequest = req.as[FetchRequest].unsafeRunSync()
      ctxLogger.info(s"FetchRequest entity received: $fetchRequest")
      ...
      request.foreach { fetch => logger.info(s"Returning fetch response to user $fetch") }
      ctxLogger.info(s"Is request completed? ${request.isCompleted}")(fetchRequest)

      val awaitable = Try(IO.fromFuture { IO { request } }.unsafeRunSync())

      ctxLogger.info(s"Awaitable: $awaitable ")(fetchRequest)
      //logger.info(s"Final result $result")
      //val value = request.value
      ctxLogger.info(s"Is completed ${request.isCompleted}")
      ctxLogger.info(s"Value ${request.value}")(fetchRequest)
    ...
    case req @ GET -> Root / "fetchGet" => {
      ...
      logger.info(s"FetchRequest entity received: $fetchRequest")
